...
async fn websocket(stream: WebSocket, state: Arc<AppState>) {
    ...
    while let Some(Ok(message)) = receiver.next().await {
        if let Message::Text(name) = message {
            ...
            {
                // If user_id that is sent by client is not taken, fill user_id string.
                ...
                if !room.user_set.contains(&connect.user_id) {
                    room.user_set.insert(connect.user_id.to_owned());
                    user_id = connect.user_id.clone();
            ...
            if tx.is_some() && !user_id.is_empty() {
                break;
            } else {
                // Only send our client that user_id is taken.
                let _ = sender
                    .send(Message::Text(String::from("UserID already taken.")))
                    .await;
    ...
    // Remove user_id from map so new clients can take it.
    rooms.get_mut(&channel).unwrap().user_set.remove(&user_id);
